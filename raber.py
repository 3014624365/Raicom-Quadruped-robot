...
class RadarPositionCorrection:
    ...
    def create_visualization(self, binary_img, line, correction):
        ...
        if correction is not None:
            ...
            status = "✓ 位置正确" if correction['is_position_ok'] else "✗ 需要调整"

            info_lines = [
                f"后方检测范围: {self.rear_angle_min}°-{self.rear_angle_max}° (正后方)",
                f"水平方向: {h_offset:.1f} 毫米 {'←' if h_offset > 0 else '→' if h_offset < 0 else ''}",
                f"垂直方向: {v_offset:.1f} 毫米 {'↑' if v_offset > 0 else '↓' if v_offset < 0 else ''}",
                f"角度: {a_offset:.1f}° {'↺' if a_offset > 0 else '↻' if a_offset < 0 else ''}",
                f"距离: {correction['distance_to_line']:.1f} 毫米",
                f"目标: {self.target_distance_from_line} 毫米",
                f"线条角度: {correction.get('center_angle', 0):.1f}°",
                status
            ]
        else:
            info_lines = [
...
def run_distance(aim_distance, max_line_distance, angle_min=140, angle_max=220):
    ...
    result = run_radar(
        ...
        collection_time=2.0,
        rear_angle_min=angle_min,
        rear_angle_max=angle_max
    )
