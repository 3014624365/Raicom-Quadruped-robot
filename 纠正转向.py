import time
from xgolib import XG

class AdaptiveYawController:
    """自适应陀螺仪控制器，提供角度校正和精确转向功能"""
    ...
    def __init__(self, dog):
        ...
        self.dog = dog
        # 转向校正系数
        self.left_correction = 1.52  # 左转校正
        self.right_correction = 1.15  # 右转校正
    ...
    def turn_by_angle(self, angle_change, speed=12, timeout=15):
        """
        ...
        返回:
            (success, actual_change): 成功标志和实际转向角度
        """
        # 标准化角度到-180到+180范围
    ...
    def turn_to_target_yaw(self, target_yaw, speed=12, error_threshold=5):
        ...
        返回:
            (success, angle_change): 成功标志和实际转向角度
        """
        # 获取当前稳定角度
    ...
    def set_reference_yaw(self, new_reference=None):
        ...
        参数:
            new_reference: 新的参考角度值，如果为None则使用当前角度
        """
...
def initialize_controller(dog):
    ...
    if _controller_instance is None:
        print("首次初始化陀螺仪控制器...")
        _controller_instance = AdaptiveYawController(dog)
    else:
        print("陀螺仪控制器已存在，重新设置参考方向...")
        _controller_instance.set_reference_yaw()
...
def get_controller():
    ...
    返回:
        controller: 陀螺仪控制器实例，如果未初始化则返回None
    """
    return _controller_instance
...
def turn(angle, speed=12):
    ...
    if _controller_instance is None:
        raise RuntimeError("请先调用initialize_controller()初始化控制器")

    return _controller_instance.turn_by_angle(angle, speed)


def turn_to_angle(target_angle, speed=12):
    """
    转向到指定的绝对角度
    ...
    return _controller_instance.turn_to_target_yaw(target_angle, speed)


def check_and_correct():
    """
    检查当前角度是否偏离参考角度，如有必要进行校正
    ...
    return _controller_instance.check_and_correct_orientation()
